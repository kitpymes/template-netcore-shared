...
namespace Kitpymes.Core.Shared.Tests
{
    ...
    public class EnumsTests
    {
        ...
        {
            var result = Util.Enums.ToList<FakeEnum>(typeof(Fakes.FakeResource));

            var (value, text, selected) = result.First(x => x.value == (int)FakeEnum.C);

            Assert.AreEqual(text, Fakes.FakeResource.C);
            Assert.IsFalse(selected);
        }
        ...
        {
            var result = Util.Enums.ToList<FakeEnum>(typeof(Fakes.FakeResource), FakeEnum.B);

            var (value, text, selected) = result.First(x => x.value == (int)FakeEnum.B);

            Assert.AreEqual(text, Fakes.FakeResource.B);
            Assert.IsTrue(selected);
        }
